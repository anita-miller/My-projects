...
int game_check_deadends(const game_info_t* info,
	...
	pos_t position = state -> pos[color];
	int x, y;
	int goal_x, goal_y;
	int neighbour_x, neighbour_y;
	

	pos_get_coords(position, &x, &y);
	pos_get_coords(info -> goal_pos[color], &goal_x, &goal_y);
	
	int num_free = game_num_free_coords(info , state, x, y);
	
	for (int dir=0; dir<4; ++dir) {
		pos_t neighbor_pos = offset_pos(info, x, y, dir);
		pos_get_coords(neighbor_pos, &neighbour_x, &neighbour_y);
		
		//if neighbour position is the goal position consider it free and
		// add one to num_free
		if (neighbor_pos != INVALID_POS &&
		    state->cells[neighbor_pos] == 3) {
			++num_free;
		}
		
		//if neighbour position is the starting point consider it free and
		// add one to num_free
		if (neighbor_pos != INVALID_POS &&
		    state->cells[neighbor_pos] == 2) {
			++num_free;
		}
