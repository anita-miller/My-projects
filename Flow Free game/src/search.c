...
int game_dijkstra_search(const game_info_t* info,
	...
	while(solution_node == NULL && !heapq_empty(&pq)){
		
		//Remove node from Queue, in order to generate its successors
		
		tree_node_t* current_node = heapq_deque(&pq);
		
		//Get next color to explore its 4 directions
		//(use game_next_move_color function in engine.h)
		int next_color = game_next_move_color(info, &current_node -> state);
	    
		int dir;
		for(dir = 0; dir<4; dir++){
		
			if(game_can_move(info, &current_node -> state, next_color, dir)){
			...
			//Within the rules of the game (see engine.h)
				...
				//Create child node
				tree_node_t* child = node_create(current_node, info, &current_node -> state);
